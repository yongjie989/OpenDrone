...
#define VBATT_CELL_FULL_MAX_DIFF 14
...
static uint8_t batteryCellCount = 3;
...
static 
# 62 "./src/main/sensors/battery.c" 3 4
      _Bool 
# 62 "./src/main/sensors/battery.c"
...
# 62 "./src/main/sensors/battery.c"
                                               ;
static 
# 63 "./src/main/sensors/battery.c" 3 4
      _Bool 
# 63 "./src/main/sensors/battery.c"
...
static uint16_t vbatLatestADC = 0;
static uint16_t amperageLatestADC = 0;
...
extern const batteryConfig_t pgResetTemplate_batteryConfig; batteryConfig_t batteryConfig_System; batteryConfig_t batteryConfig_Copy; extern const pgRegistry_t batteryConfig_Registry; const pgRegistry_t batteryConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 11 | (1 << 12), .size = sizeof(batteryConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&batteryConfig_System, .copy = (uint8_t*)&batteryConfig_Copy, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_batteryConfig}, };

const batteryConfig_t pgResetTemplate_batteryConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .voltage = { .scale = 1100, .cellMax = 424, .cellMin = 330, .cellWarning = 350 }, .current = { .offset = 0, .scale = 400, .type = CURRENT_SENSOR_ADC }, .capacity = { .value = 0, .warning = 0, .critical = 0, .unit = BAT_CAPACITY_UNIT_MAH, } }
# 100 "./src/main/sensors/battery.c"
 ;
...
uint16_t batteryAdcToVoltage(uint16_t src)
{
    ...
    return((uint64_t)src * batteryConfig()->voltage.scale * 3300 / (0xFFF * 1000));
}

int32_t currentSensorToCentiamps(uint16_t src)
{
    int32_t microvolts = ((uint32_t)src * 3300 * 1000) / 0xFFF - (int32_t)batteryConfig()->current.offset * 1000;
    return microvolts / batteryConfig()->current.scale;
}
...
void batteryUpdate(uint32_t vbatTimeDelta)
{
    ...
    if (batteryState == BATTERY_NOT_PRESENT && vbat > 100)
    {
        ...
        batteryFullWhenPluggedIn = batteryAdcToVoltage(vbatLatestADC) >= (batteryFullVoltage - cells * 14);
    ...
    else if (batteryState != BATTERY_NOT_PRESENT && vbat <= 100) {
        batteryState = BATTERY_NOT_PRESENT;
        batteryCellCount = 0;
        batteryWarningVoltage = 0;
        batteryCriticalVoltage = 0;
    ...
    if (batteryState != BATTERY_NOT_PRESENT) {
        ...
        if (batteryUseCapacityThresholds) {
            ...
            {
                case BATTERY_OK:
                    if (vbat <= (batteryWarningVoltage - 10))
                        batteryState = BATTERY_WARNING;
                    break;
                case BATTERY_WARNING:
                    if (vbat <= (batteryCriticalVoltage - 10)) {
                        batteryState = BATTERY_CRITICAL;
                    } else if (vbat > (batteryWarningVoltage + 10)){
                        batteryState = BATTERY_OK;
                    }
                    break;
...
# 227 "./src/main/sensors/battery.c" 3 4
_Bool 
# 227 "./src/main/sensors/battery.c"
    batteryWasFullWhenPluggedIn(void)
{
...
# 232 "./src/main/sensors/battery.c" 3 4
_Bool 
# 232 "./src/main/sensors/battery.c"
    batteryUsesCapacityThresholds(void)
{
...
# 237 "./src/main/sensors/battery.c" 3 4
_Bool 
# 237 "./src/main/sensors/battery.c"
    isBatteryVoltageConfigured(void)
{
...
# 275 "./src/main/sensors/battery.c" 3 4
_Bool 
# 275 "./src/main/sensors/battery.c"
    isAmperageConfigured(void)
{
...
void currentMeterUpdate(int32_t timeDelta)
{
    ...
    switch (batteryConfig()->current.type) {
        case CURRENT_SENSOR_ADC:
            ...
            amperageLatestADC = pt1FilterApply4(&amperageFilterState, amperageLatestADC, 1, timeDelta * 1e-6f);
